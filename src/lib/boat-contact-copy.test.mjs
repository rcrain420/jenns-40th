...
describe("boat contact nudge", () => {
  it("tells the registrant they are not a paid seat unless they add themselves", () => {
    const text = boatContactNotAnglerNudge(formatUsdWhole(BOAT_ENTRY_CENTS));
    assert.match(text, /boat contact/i);
    assert.match(text, /add yourself as an angler/i);
    assert.match(text, /\$300 boat entry/);
