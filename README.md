...
Set `EVENT_PIN` in `.env` to exercise the event-unlock fallback. `/register/success` still shows the unlock link next to the teammate invite, so unlock does not depend on mail.
