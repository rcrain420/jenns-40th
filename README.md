...
- Teams of 1–4 adult anglers; a main-tournament boat needs at least one adult; guided (optional captain) or non-guided (optional primary contact)
- Youth register separately for free RowRide from land (`/register/youth`) — they are not added to a boat roster (app safety max 8 youth on a land entry). On event day they may fish from a boat if the captain or guide allows it. Youth rules live on `/kids#rules` (`docs/rowride-rules.md`); adult boat rules stay on `/rules`.
- $300 flat per boat; youth do not change the boat entry; side pots $50 per team, per pot; Venmo payment link + QR (`public/venmo-qr.png`)
